...
contract CustomDex {
    ...
    function swapTokenToToken(string memory srcTokenName, string memory destTokenName, uint256 amount) public {
        require(tokenInstanceMap[srcTokenName].transferFrom(msg.sender, address(this) , amount));

        require(tokenInstanceMap[destTokenName].transfer(msg.sender, amount));
